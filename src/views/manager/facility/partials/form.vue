<script setup>
/** Imports */
import { createFacility, deleteFacility, readFacility, updateFacility } from '@/services/facilityService';
...
/** Functions */
const loadFacilityData = async (uuid) => {
    try {
        const res = await readFacility(uuid);
        facilityData.value = res.data;
        console.log('facilityData: ', facilityData.value);
    } catch (error) {
        toast.add({ severity: 'error', summary: 'Fetch Data Gagal!', detail: 'sysMsg.value', life: 5000 });
    }
};
...
const handleDelete = async () => {
    if (!form.uuid) {
        toast.add({ severity: 'warn', summary: 'Gagal menghapus data!', detail: 'Data lapas tidak valid.', life: 5000 });
        return;
    }

    const res = await deleteFacility(form.uuid);
...
<template>
    <form v-if="props.action === 'register' || props.action === 'update'" @submit.prevent="handleSubmit" class="flex flex-col gap-4">
        <div class="flex flex-col gap-2">
            <label v-text="'Nama'" />
            <InputText v-model="form.name" placeholder="Masukkan nama lapas" type="text" class="min-w-80" />
        </div>
        <div class="flex flex-col gap-2">
            <label v-text="'Alamat'" />
            <InputText v-model="form.address" placeholder="Masukkan alamat lapas" type="text" class="min-w-80" />
        </div>
        <hr />
    ...
    <div v-if="props.action === 'delete'">
        <p>Apakah Anda yakin ingin menghapus data lapas <b v-text="form.name" class="text-red-500" />?</p>
        <hr />
        <div class="flex justify-end gap-2">
            <Button @click="emits('close')" type="button" icon="pi pi-angle-left" label="Batalkan" severity="secondary" />
